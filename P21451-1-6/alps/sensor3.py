...
class NtfyDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data): 
        ...
        if int((cal[0:2]), 16) == 0xE0:
            ...
            file_path='./data_save/battery/sensor3/'+hour_now+".json"

            server_path = 'SmaAgri/Noken/sonoda/'
            trans_file = trans_hour + '.json'
            path_battery = './data_save/battery/sensor3/'
        ...
        if int((cal[0:2]), 16) == 0xf3:
            ...
            file_path='./data_save/environment/sensor3/'+hour_now+".json"
            ...
            if os.path.isfile(file_path):
                ...
                path_environment = './data_save/environment/sensor3'
                server_path = 'SmaAgri/Noken/sonoda/'
                trans_hour = trans_hour.strftime("%Y-%m-%d-%H")
                trans_file_path = './data_save/environment/sensor3'+trans_hour+'.json'
                trans_file = trans_hour+'.json'
...
# Main loop --------
        while True:
            if AlpsSensor(sensor3).waitForNotifications(1.0):
