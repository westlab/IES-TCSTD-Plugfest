...
class NtfyDelegate(btle.DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data): 
        ...
        if int((cal[0:2]), 16) == 0xE0:
            ...
            file_path='./data_save/battery/'+hour_now+".json"

            server_path = 'SmaAgri/Noken/sonoda/'
            trans_file = trans_hour + '.json'
            path_battery = './data_save/battery/'
        ...
        if int((cal[0:2]), 16) == 0xf3:
            ...
            if os.path.isfile(file_path):
                ...
                server_path = 'SmaAgri/Noken/sonoda/'
                trans_hour = trans_hour.strftime("%Y-%m-%d-%H")
                trans_file_path = './data_save/environment/'+trans_hour+'.json'
                trans_file = trans_hour+'.json'
...
def main():
        sensor1 = '48:F0:7B:78:4B:98'
        sensor2 = '48:F0:7B:78:4B:81'
#    sensor3 = '48:F0:7B:78:4B:70'
#    sensor4 = '48:F0:7B:78:4B:8E'
#    sensor5 = '48:F0:7B:78:4B:8D'
#    sensor6 = '48:F0:7B:78:4B:6E'
#        sensor7 = '48:F0:7B:78:4B:6F'

#    Sensors = [sensor1, sensor2, sensor3, sensor4, sensor5, sensor6, sensor7]
    
#    for sensor in Sensors:

        alps = AlpsSensor(sensor1)
...
#    alps.writeCharacteristic(0x0018, struct.pack('<bbb', 0x21, 0x03, 0x01), True)
 #   alps.writeCharacteristic(0x0018, struct.pack('<bbbb', 0x10, 0x04, 0x01, 0x00), True)
        ...
        test = AlpsSensor(sensor2)
        test.setDelegate( NtfyDelegate(btle.DefaultDelegate) )
# Main loop --------
        while True:
            if test.waitForNotifications(1.0):
